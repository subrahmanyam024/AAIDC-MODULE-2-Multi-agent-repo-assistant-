    ...
    if project_mode == "Create New Project":
        ...
        with col1:
            ...
            if github_url and project_name:
                if st.button("📥 Download & Analyze Repository", use_container_width=True):
        ...
        with col2:
            if st.session_state.repo_data and st.session_state.new_project_analyzed:
                ...
                st.metric("Languages Found", len(repo_data["structure"]["languages"]))
                
                project_type = repo_data["analysis"].get("project_type", "Unknown")
                st.metric("Project Type", project_type)
                st.metric("Dependencies", len(repo_data["analysis"].get("dependencies", {})))
                ...
                with st.expander("View repository structure", expanded=False):
                    ...
                    if len(repo_data["structure"]["files"]) > 20:
                        st.write(f"... and {len(repo_data['structure']['files']) - 20} more files")
                
                st.markdown("**🔧 Languages**")
                if repo_data["structure"]["languages"]:
                    st.write(", ".join(repo_data["structure"]["languages"]))
                else:
                    st.write("No code files found")
        ...
        if st.session_state.current_project_id:
            ...
            if project_data:
                ...
                if project_data.get("repo_data"):
                    ...
                    st.metric("Languages Found", len(structure.get("languages", [])))
                    
                    project_type = analysis.get("project_type", "Unknown")
                    st.metric("Project Type", project_type)
                    st.metric("Dependencies", len(analysis.get("dependencies", {})))
                    ...
                    with st.expander("View repository structure", expanded=False):
                        ...
                        if len(structure.get("files", [])) > 20:
                            st.write(f"... and {len(structure.get('files', [])) - 20} more files")
...
with tab2:
    if st.session_state.repo_data:
        st.markdown("### ✍️ Generation Instructions")
...
"""
                            ...
                            outline = article_gen.generate_outline(repo_context, generation_instructions)
